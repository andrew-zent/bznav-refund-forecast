...
SEASON_ADJUSTMENT = {
    1: +0.03,   # 연초 — 0.00→+0.03: 연속 과소추정(11~3월) 하방압력 부분 보정
    2: 0.00,    # 설 연휴 — +0.03→0.00: Feb는 원래 과대추정(+7.7%); +0.03 적용 시 +16.8% 신규 이상치 발생, streak은 Nov/Jan 조정으로 이미 차단
    3: +0.13,   # 1Q 폭증 — +0.10→+0.13: 2026-03 백테스트 -15.3%(±15% 임계 초과) → 약 -13%로 개선
    4: 0.00,    # 평월
    5: 0.00,    # 종소세 신고기
    ...
    9: 0.00,    # 평월
    10: 0.00,   # 평월
    11: +0.05,  # 연말 전월 — +0.02→+0.05: Nov-Mar 5개월 연속 과소추정 스트릭 차단 지원
