...
<div class="meta"><span>갱신: {gen}</span><span>MAPE {mape}%</span><span>데이터: {data['total_claims']:,}건</span></div>

<div class="kpi-row">
{''.join(f'<div class="kpi hl"><div class="kpi-label">{f["month"]} 권장</div><div class="kpi-value">{f["adjusted"]}<span style="font-size:12px;color:#8b949e">억</span></div><div class="kpi-sub">모델 {f["total"]}억, 시즌 {f["season_adj"]:+.0%}</div></div>' for f in fc[:3])}
...
<div class="card"><h2>🎯 권장 예측값</h2>
<table><thead><tr><th>월</th><th>정기</th><th>추심</th><th>총합</th><th>시즌</th><th>권장</th></tr></thead>
...
{{label:'모델 예측',data:{fc_total},backgroundColor:'#3fb950',borderRadius:4}},
{{label:'시즌 보정',data:{fc_adj},type:'line',borderColor:'#d29922',backgroundColor:'transparent',tension:.3,pointRadius:6,pointStyle:'rectRot',borderWidth:2.5}}
